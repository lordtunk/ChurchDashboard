  ...
  if($dict['success'] == TRUE) {
    ...
    try {
      ...
      $query = "UPDATE
                ...
                WHERE
                  id=:id";
      $results = $f->executeAndReturnResult($query, array(":first_name"=>$person->first_name, ":last_name"=>$person->last_name, ":description"=>$person->description, ":adult"=>$person->adult, ":active"=>$person->active, ":modified_by"=>$user_id, ":id"=>$person->id));
      $dict['success'] = TRUE;
    } catch (Exception $e) {
      $dict['success'] = FALSE;
